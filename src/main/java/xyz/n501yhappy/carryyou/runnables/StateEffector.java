...
public class StateEffector implements Runnable { //用于给予玩家特殊状态
    ...
    @Override
    public void run() {
        for (UUID uuid : chickenState.getWithTarget()) {
            Entity entity = Bukkit.getEntity(uuid);
            if (entity == null) {
                chickenState.remove(uuid);
                continue;
            }
            if (!(entity instanceof LivingEntity)) continue;
            Version.getAdapts().EntityScheduler_execute(CarryYou.instance,entity,() -> {
                ((LivingEntity) entity).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, DURATION+10, 1,true,false));
            });
        }
    }
}
